...
from lib.recipe import get_menu_data, register_dish_history
    ...
    for date_key in st.session_state.dates.keys():
        ...
        with cols[1]:
            ...
            st.session_state.day_to_register_dish[date_key] = st.selectbox(
                "主菜",
                options=[""] + dish_list,
                index=(dish_list.index(st.session_state.day_to_register_dish[date_key]) + 1 if st.session_state.day_to_register_dish[date_key] else 0),
